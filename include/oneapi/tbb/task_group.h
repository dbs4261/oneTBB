...
public:
    ...
    function_task(FF&& f, d1::wait_tree_node_interface* node, d1::task_group_context& ctx, d1::small_object_allocator& alloc)
...
class function_stack_task : public d1::task {
    const F& m_func;
    d1::wait_tree_node_interface* m_wait_tree_node;
...
public:
    function_stack_task(const F& f, d1::wait_tree_node_interface* node) : m_func(f), m_wait_tree_node(node) {
...
protected:
    d1::wait_context_node m_wait_node;
    d1::task_group_context m_context;

    template<typename F>
    task_group_status internal_run_and_wait(const F& f) {
        function_stack_task<F> t{ f, r1::get_thread_reference_node(&m_wait_node) };
    ...
    d1::task* prepare_task(F&& f) {
        ...
        return alloc.new_object<function_task<typename std::decay<F>::type>>(std::forward<F>(f),
            r1::get_thread_reference_node(&m_wait_node), context(), alloc);
    }
